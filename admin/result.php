...
session_start();
   ...
   $std = $handle->query("select COUNT(*) AS total_std from student where cls_id = '$cls_id' and section='$sec' " );
...
$num_posi = mysqli_num_rows($posi_check);
    ...
    if($num_posi > 0){
       ...
       $posiupdat = $handle->query("UPDATE posi set std_avg = '$std_avg'
                where std_id = '$std_id' and cls_id = '$cls_id' and term_id = '$ses' and ses_id='$term' and section='$sec' ");

       //updating position
        $update = $handle->query("UPDATE posi SET position = @curRank:=0 WHERE cls_id = '$cls_id' and ses_id = '$ses'
    ...
    WHERE cls_id = '$cls_id' and ses_id = '$ses'
    ...
    $std_position = $handle->query("insert into posi(std_id,term_id,ses_id,cls_id,std_avg,section)
             values('$std_id','$term','$ses','$cls_id','$std_avg','$sec')");
...
<div class="container">
    <?php
        if($_GET['checker'] == "std"){
   ?>
     <a href="std_dashboard.php?dash" class="btn btn-warning">Back</a>
    <?php
        }else if($_GET['checker'] == "admin"){
        ?>
    <a href="admin-dashboard.php?re" class="btn btn-warning">Back</a>
<?php    
        }else if($_GET['checker'] == "staff") {
        ?>
    <a href="staff-dashboard.php?re" class="btn btn-warning">Back</a>
    <?php
    }
    ?>
    <span style="float: right"><a target="_blank" href="print_result.php?std_id=<?=$handle->validate($std_id)?>
    &cls=<?=$handle->validate($cls_id);?>
    &term=<?=$handle->validate($term);?>&ses=<?=$handle->validate($ses);?>
    &sec=<?=$handle->validate($sec);?>" class="btn btn-success">PRINT</a></span>
